...
enum Sound_States {S_SMstart, S_press_UP, S_press_DOWN, S_release, S_ring};
...
int TickFct_SoundGenerate(int state) {
	switch(state) {
		case S_SMstart:
		if (!GetBit(PINA, 0) && GetBit(PINA, 1))state = S_press_UP;
		else if (GetBit(PINA, 0) && !GetBit(PINA, 1))state = S_press_DOWN;
		else if (!GetBit(PINA, 2) && GetBit(PINA, 0) && GetBit(PINA, 1)) state = S_ring;
		else state = S_SMstart;
		break;
		case S_press_UP:
		if (GetBit(PINA, 0))
		{
			state = S_release;
			if (frequency < 500) frequency = 1000 / (((1000 / frequency / 2) - 2) * 2);
		}
		break;
		case S_press_DOWN:
		if (GetBit(PINA, 1))
		{
			state = S_release;
			if (frequency > 10) frequency = 1000 / (((1000 / frequency / 2) + 2) * 2);

		}
		break;
		case S_ring: if (GetBit(PINA, 2)) state = S_SMstart;
		break;
	...
	switch(state) {
		case S_ring: set_PWM(frequency);break;
		case S_press_UP:
		case S_press_DOWN:
		case S_release:
		case S_SMstart:
