...
void Tick2()
{
	...
	
	{
		...
		case plus_P: if (GetBit(PINA, 0)) {state2 = plus_R; break;}
					 else if (!GetBit(PINA, 0) && !GetBit(PINA, 1)) {state2 = reset;break;}
					 else state2 = plus_P;break;
		case minus_P:if (GetBit(PINA, 1)) {state2 = minus_R; break;}
				     else if (!GetBit(PINA, 0) && !GetBit(PINA, 1)) {state2 = reset;break;}
					 else state2 = minus_P;break;
		case plus_R: counter += 1;state2 = Start2;break;
		case minus_R: counter -= 1;state2 = Start2;break;
