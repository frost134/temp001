...
void Tick()
{
	...
	switch(state)
	{
		case Start : PORTD = 0x07; break;
		case wait : break;
		case plus : if (PIND == 0x09){flag = 1;}else{PORTD = PIND +1; flag = 1;} break;
		case minus : if (PIND == 0x00){flag = 1;}else{PORTD = PIND -1; flag = 1;} break;
		case reset : PORTD = 0x00;break;
	}
}
